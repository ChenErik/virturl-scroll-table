...
export default defineComponent({
  ...
  setup(props, { slots }) {
    ...
    function genChild(item: any, index: number) {
      ...
      return (
          <div className={`${prefixCls}__item`} style={{ top }} key={index}>
    ...
    return () => (
        <div className={prefixCls} style={unref(getWrapStyleRef)} ref={wrapElRef}>
          <div className={`${prefixCls}__container`} style={unref(getContainerStyleRef)}>
            {renderChildren()}
          </div>
        </div>
    )
...
<style scoped lang="scss">
  .virtual-scroll {
    ...
    &__container {
      display: block;
      .virtual-scroll__item:nth-child(odd) {
        background-color: #f5f5f7;
      }

      .virtual-scroll__item:nth-child(even) {
        background-color: #fff;
      }
    }
    &__item {
