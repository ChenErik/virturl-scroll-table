...
for (let i = 0; i < 2000; i++)
  dd.push({ id: i, userName: `userName${i}`, realName: `realName${i}`, email: `email${i}@${i}.com`, address: `江苏南京${Math.random() * 100}` })
const data = ref<Data[]>([])
...
<template>
  <div style="display: flex;height: 100%;width: 100%;">
    <table class="my-table" style="display: flex;flex-direction: column;user-select: none;border: 3px solid red;">
      ...
      <div ref="tbodyScrollRefs" class="table-body" style="flex-grow: 1;overflow: auto;position: relative;" @scroll="onScroll($event)">
        ...
        >
          <tr v-for="(item, index) in showList" :key="index" style="height: 25px;">
            <td>
              <div h-25px>
                {{ item.id }}
              </div>
